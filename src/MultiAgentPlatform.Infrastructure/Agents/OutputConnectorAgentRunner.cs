...
public class OutputConnectorAgentRunner : IAgent
{
    private readonly IEnumerable<ConnectorDefinition> _connectors;
    private readonly IConnectorFactory _connectorFactory;

    public OutputConnectorAgentRunner(AgentDefinition definition, IEnumerable<ConnectorDefinition> connectors, IConnectorFactory connectorFactory)
    {
    ...
    {
        var relevant = _connectors.Where(c => c.IsEnabled && c.Direction == ConnectorDirection.Output).ToList();
        if (relevant.Count == 0)
        {
        ...
        if (successCount == 0)
        {
            return AgentResult.Fail($"All output connectors failed: {string.Join("; ", errors)}");
        }
